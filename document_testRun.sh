...
TEST_RUN="test_results/fork_test_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$TEST_RUN"

# Copy all monitoring data
cp -r test_results/live_monitoring/* "$TEST_RUN/"

# Create test report

# Fork Test Report

cat > "$TEST_RUN/TEST_REPORT.md" << 'EOF'
## Test Configuration
- **Date**: $(date)
- **Duration**: [Fill in]
- **Total Nodes**: 8
...
echo "Test report template created at: $TEST_RUN/TEST_REPORT.md"
