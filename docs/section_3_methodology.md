...
## 13. Phase 2: Scenario Discovery

Phase 2 applies scenario discovery methods to identify parameter regions where fork
outcomes are most uncertain — the "transition zone" between clear v27 wins and clear
v26 wins. This section documents the methodology and preliminary findings.

For detailed results, see `warnetScenarioDiscovery/docs/phase2_scenario_discovery_preliminary.md`.

### 13.1 Objective

Identify the parameter subspace where:
1. Fork outcomes are closest to 50/50 (maximum uncertainty)
2. Contentiousness is highest (reorgs, economic lag, prolonged competition)
3. Small parameter changes can flip the outcome

This transition zone is where soft fork dynamics are most sensitive and where
additional simulation effort should be focused.

### 13.2 Key Sweep Parameters

Phase 1 sweeps identified four parameters with causal influence on fork outcomes:
...
Two retarget intervals are tested to understand how difficulty adjustment speed
affects fork dynamics:

| Regime | `--retarget-interval` | Real-world Analog | Behavior |
|--------|----------------------|-------------------|----------|
| **144-block** | 144 | ~1 day (accelerated) | Fast difficulty response; minority chain recovers quickly |
| **2016-block** | 2016 | ~2 weeks (Bitcoin mainnet) | Slow difficulty response; prolonged hashrate imbalance |

The 2016-block regime more closely matches Bitcoin mainnet behavior but requires
longer simulation runs to observe difficulty adjustments.

### 13.4 Analysis Methods

#### Patient Rule Induction Method (PRIM)

PRIM finds axis-aligned "boxes" (hyperrectangles) in parameter space that maximize
a target metric. Two modes are used:

- **Uncertainty mode**: Maximize `1 - 2|mean - 0.5|` to find regions where outcomes
  are closest to 50/50
- **Contentiousness mode**: Maximize composite contentiousness score

PRIM iteratively peels away parameter ranges that reduce the target, producing
interpretable bounds like "uncertainty is highest when `pool_committed_split ≤ 0.50`".

#### Random Forest Classification

A random forest classifier predicts fork outcome (v27_dominant vs v26_dominant)
from the four key parameters. Feature importance scores reveal which parameters
most strongly determine outcomes.

#### Logistic Regression with Interactions

Logistic regression with polynomial interaction terms captures parameter interactions
(e.g., `pool_ideology_strength × pool_max_loss_pct`) that may not appear in
single-parameter analysis.

### 13.5 Findings (Phase 2 Full Analysis, April 2026)

#### Data Coverage

696 full-network (60-node) scenarios across both regimes:

| Regime | Scenarios | v27 Win Rate |
|--------|-----------|--------------|
| 144-block | 268 | 44.0% |
| 2016-block | 298 | 54.0% |
...
Random forest feature importance (OOB accuracy: 80.0% at 144-block, 83.2% at 2016-block):

| Parameter | 144-block | 2016-block |
|-----------|-----------|------------|
| `economic_split` | **77.2%** | 20.2% |
| `pool_committed_split` | 11.3% | **52.8%** |
| `pool_ideology_strength` | 6.1% | — |
| `pool_max_loss_pct` | 8.1% | — |

**Key finding:** The dominant causal variable *reverses* between regimes. `economic_split`
controls outcomes at 144-block; `pool_committed_split` becomes the dominant factor at
2016-block (52.8% importance), with `economic_split` dropping to 20.2%. This causal
inversion is the central quantitative result of Phase 2.

#### PRIM Uncertainty Bounds

**144-block:** Full-network grid data is too non-uniform for PRIM box-finding; RF feature
importances above are valid but axis-aligned bounds are not well-defined.

**2016-block PRIM uncertainty zone** (Phase 3 LHS target region):
...
Saved to `tools/discovery/output/2016/uncertainty_bounds.yaml`.

#### Inversion Zone

Decision boundary visualization reveals an "inversion zone" around
`economic_split ≈ 0.60–0.70` and `pool_committed_split ≈ 0.30–0.50` where
*increasing* pool commitment to v27 paradoxically *decreases* v27's win probability.

This is the Foundry flip-point mechanism (§4.3.2): below ~0.214 committed split,
Foundry USA is economically trapped on v26 and its forced exit triggers a neutral
pool cascade. Above the flip-point, Foundry shifts to v27-committed ideology,
which purifies the v26 defending block and breaks the cascade mechanism.

#### Regime Comparison

| Metric | 144-block | 2016-block | Difference |
|--------|-----------|------------|------------|
| v27 win rate | 44.0% | 54.0% | +10% |
| RF OOB accuracy | 80.0% | 83.2% | +3.2% |
| Mean contentiousness | 0.132 | 0.271 | +0.139 |

Contentiousness is 2× higher at 2016-block, consistent with the slower difficulty
response sustaining prolonged hashrate competition before resolution.

### 13.6 Contentiousness Score

Contentiousness measures how "fought" a fork outcome was, independent of which
side won. It is a composite of four normalized metrics:

```
contentiousness = 0.25 * norm_reorgs
                + 0.25 * norm_reorg_mass
                + 0.25 * (1 - norm_cascade_time)
                + 0.25 * norm_econ_lag
```

Where:
- `norm_reorgs`: Total reorg count / max observed
- `norm_reorg_mass`: Total blocks displaced by reorgs / max observed
- `norm_cascade_time`: Time for hashrate cascade to complete (inverted: faster = more contentious)
- `norm_econ_lag`: Delay between hashrate cascade and economic node switching

High contentiousness indicates prolonged competition, frequent chain reorganizations,
and delayed economic resolution — conditions of practical concern for network
participants.
...
|------|----------|---------|
| `fit_boundary.py` | `warnetScenarioDiscovery/tools/discovery/` | Main analysis script |
| `sweep_results.db` | `warnetScenarioDiscovery/tools/sweep/` | SQLite database of all scenarios |
| Output files | `warnetScenarioDiscovery/tools/discovery/output/` | YAML bounds, JSON comparisons, figures |

```bash
# Run analysis with uncertainty optimization
